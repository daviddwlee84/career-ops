...
| `offer.md` | `modes/offer.md` (EN) | Evaluation complete d'une offre (Blocs A-F) |
| `apply.md` | `modes/apply.md` (EN) | Assistant live pour remplir les formulaires de candidature |
| `pipeline.md` | `modes/pipeline.md` (EN) | Inbox d'URLs / Second Brain pour les offres collectees |

Les autres modes (`scan`, `batch`, `pdf`, `tracker`, `auto-pipeline`, `deep`, `contact`, `compare`, `project`, `training`) restent en anglais dans `modes/`. Leur contenu est surtout du tooling, des chemins et des commandes -- il doit rester independant de la langue.
