...
Process job URLs accumulated in `data/pipeline.md`. The user adds URLs whenever they want, then runs `/career-ops pipeline` to process them all.
...
3. **WebSearch (last resort):** Search secondary boards that index the JD.
...
If anything is out of sync, warn the user before continuing.
